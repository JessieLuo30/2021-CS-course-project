...
public class BinarySearchTreeMap<K extends Comparable<K>, V> implements OrderedMap<K, V> {
  ...
  private Node<K, V> remove(Node<K, V> subtreeRoot, Node<K, V> node) {
    int cmp = subtreeRoot.key.compareTo(node.key);
    if (cmp == 0) {
      return remove(subtreeRoot);
    } else if (cmp > 0) {
      subtreeRoot.left = remove(subtreeRoot.left, node);
    } else {
      subtreeRoot.right = remove(subtreeRoot.right, node);
    }
    return subtreeRoot;
  }
